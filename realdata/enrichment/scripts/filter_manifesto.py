...
def merge_df_count_branch_mut(countmerge, snpinfo, classify_rule=1):
    if countmerge.endswith("txt"):
        ...
        raise ValueError(
            "Unsupported file format for countmerge. Please provide a .txt or .csv file."
        )
    ...
    for i in data.index:
        ...
        for j in range(len(mks)):
            if (
                ons[j] == "on"
                and mks[j]
                in ["ND00_strict", "ND10_strict", "ND01_strict", "ND11_strict"]
                and snpinfo_dict[snps[j]]
            ):
                tot += 1
                ...
                if float(yri[j]) > 0:
                    nyri += 1
            if (
                mks[j] in ["ND00_strict", "ND10_strict", "ND01_strict", "ND11_strict"]
                and not snpinfo_dict[snps[j]]
            ):
                if mks[j] == "ND00_strict":
                    new_mks += "ND00"
                elif mks[j] == "ND10_strict":
    ...
    if classify_rule == 1:
        data.loc[
            (data["nd10_prop"] > 0.2)
            & (data["nd10_b"] > data["nd01_b"])
            & (data["nderived_strict"] >= 30)
            & (
                data[["ND00_strict", "ND10_strict", "ND01_strict", "ND11_strict"]].sum(
                    axis=1
                )
                >= 10
            ),
        ...
        data.loc[
            ((data["nd00_prop"] <= 0.8) | (data["nYRI"] <= 0.1))
            & (data["nd10_b"] > data["nd01_b"])
            & (data["nderived_strict"] >= 30)
            & (
                data[["ND00_strict", "ND10_strict", "ND01_strict", "ND11_strict"]].sum(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Filter annotated tables from TRACE with manifesto filter."
    )

    # Add arguments with prefixes
    parser.add_argument(
        "--input", required=True, help="Input table file (.count_merge.txt file)"
    )
    parser.add_argument(
        "--snpinfo-pref",
        required=True,
        help="prefix to snpinfo file with manifesto info appended (parts before .[chrom].txt)",
    )
    parser.add_argument(
        "--output", required=True, help="Output file for the filtered table"
    )
    parser.add_argument(
        "--classify-group",
        type=int,
        choices=[1, 2],
        default=1,
        help="classification rule group for final annotation, 1 for more stringent classification and 2 for more relaxed classification",
    )
