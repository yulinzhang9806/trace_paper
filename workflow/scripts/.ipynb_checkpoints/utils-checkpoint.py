...
class Analysis_workflow_utils:
    ...
    def sample_ts(self, t1_h, t1_l, t2_h, t2_l, dist = 'uniform', seed = 1):
        np.random.seed(seed)
        if dist == 'uniform':
            t1 = np.random.uniform(t1_l, t1_h)
            t2 = np.random.uniform(t2_l, t2_h)
            return t1, t2
        # elif dist == 'log':
        #     t1 = np.random.lognormal(t1_l, t1_h)
        #     t2 = np.random.lognormal(t2_l, t2_h)
        else:
            return None
    ...
    def record_hmm_results_ondind(
        ...
        ancoal, at1s, at2s, atreespan = ahmm.prepare_data_tmrca(ts = target_ts, ind = ind, t_archaic=t_archaic)
        ...
        fncoal, ft1s, ft2s, ftreespan = fhmm.prepare_data_tmrca(ts = outgroup_ts, ind = ind, t_archaic=t_archaic)
        ...
        np.savez_compressed(
            ...
            a_param=a_param,
            f_param=f_param,
            a_tree_span=ahmm.treespan,
            f_tree_span=fhmm.treespan,
            allow_pickle=True
        )
    ...
    def record_hmm_data_singer_singlets(
        ...
        def get_data(ts, ind, t_archaic, windowsize, func):
            ...
            hmm = GhostProductHmm()
            tncoal, tt1s, tt2s, treespan = hmm.prepare_data_tmrca(ts = ts, ind = ind, t_archaic=t_archaic)
            if len(ind) == 1:
                ...
                for k in range(m):
                    ...
                    if len(curtrees) == 0:
                        ...
                        if len(curtrees) == 0:
                            ...
                            ncoal_sub[i][k] = func(np.repeat(tncoal[i][curtrees], treelens))
                            t1s_sub[i][k] = func(np.repeat(tt1s[i][curtrees], treelens))
                            t2s_sub[i][k] = func(np.repeat(tt2s[i][curtrees], treelens))
                        curtrees = []
        ...
        ancoal_sub, at1s_sub, at2s_sub = get_data(target_ts, ind, t_archaic, windowsize, func)
        ...
        fncoal_sub, ft1s_sub, ft2s_sub = get_data(outgroup_ts, ind, t_archaic, windowsize, func)
...
class SUMMARIZE:
    ...
    def parse_snpinfo_anc(self, snpinfo, arc_return_dict):
        ...
        for i in range(1, len(lines)):
            ...
            if ref == anc:
                outdict["_".join([str(pos), ref, alt])] = ["keep", dnea, nea_missing, dden, den_missing, aafr, aout, instrict, freqs]
            elif alt == anc:
                outdict["_".join([str(pos), ref, alt])] = ["switch", dnea, nea_missing, dden, den_missing, aafr, aout, instrict, freqs]
            elif anc in ['A', 'T', 'C', 'G']:
                outdict["_".join([str(pos), ref, alt])] = ["other", dnea, nea_missing, dden, den_missing, aafr, aout, instrict, freqs]
            else:
                outdict["_".join([str(pos), ref, alt])] = ["N", dnea, nea_missing, dden, den_missing, aafr, aout, instrict, freqs]
        return outdict
    ...
    def count_snps(self, seg_snps, snp_dict, hap="left"):
        ...
        for i in range(len(lines)):
            ...
            ancinfo, dnea, nea_missing, dden, den_missing, aafr, aout, instrict, freqs = snp_dict[pos]
        ...
        return snpcount, snpcount_strict, np.array(dsnps), np.array(dsnps_freqs)
    ...
    def final_ind_count(self, samplename, summary, hap, snpinfo, bcfpref, outpref):
        ...
        repeatmask: str, a bed file with masked regions
        ...
        out1 = "chromosome\tstart\tend\tdsnps\tDAF_GBR\tDAF_YRI\tDAF_GBRYRI\n"
        ...
        for i in range(1, len(lines)):
            ...
            snpcount, snpcount_strict, dsnps, dsnps_freqs = self.count_snps(str(outpref) + str(i) + "seg_snps", snp_dict, hap)
    ...
    # def append_seg_freq(self, npzpref, indlist, summary, outpref):
